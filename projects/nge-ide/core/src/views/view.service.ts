  ...
  register(...views: IView[]): void {
    views.forEach((view) => {
      ...
      if (entries.find((v) => v.id === view.id)) {
        throw new Error(`There is already a view registered with the id ${view.id}`)
      }
      ...
      view.commands = combineLatest([this.viewCommandsRegistry.get(view.id)!.asObservable(), view.commands]).pipe(
        map(([a, b]) => [...a, ...b])
      )
  ...
  registerCommands(...commands: ViewCommand[]): void {
    commands.forEach((vc) => {
      ...
      if (entries.find((v) => v.id === vc.command.id)) {
        throw new Error(`There is already a command registered with the id ${vc.command.id} for view ${vc.viewId}`)
      }
