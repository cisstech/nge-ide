...
export class ViewContainerService implements IContribution {
  ...
  register(...containers: IViewContainer[]): void {
    ...
    containers.forEach((container) => {
      if (entries.find((entry) => entry.id === container.id)) {
        throw new Error(`There is already a view container registered with the id ${container.id}`)
      }
  ...
  unregister(id: string) {
    this.registry.next(this.registry.value.filter((e) => e.id !== id))
  }
  ...
  onDidOpen(...containerIds: string[]) {
    return this.openEvent.pipe(filter((e) => !containerIds.length || containerIds.includes(e)))
  }
