...
describe('StorageService', () => {
  ...
  it('should be created', inject(
    [StorageService],
    async (service: StorageService) => {
      expect(service).toBeTruthy();
      await lastValueFrom(service.clear());
    }
  ));

  it('should set and get string', inject(
    [StorageService],
    async (service: StorageService) => {
      const key = 'strKey';
  ...
  it('should set and get boolean', inject(
    [StorageService],
    async (service: StorageService) => {
      const key = 'boolKey';
      const value = false;
  ...
  it('should set and get number', inject(
    [StorageService],
    async (service: StorageService) => {
      const key = 'numberKey';
      const value = 42;
  ...
  it('should set and get item', inject(
    [StorageService],
    async (service: StorageService) => {
      const key = 'strKey';
      ...
      expect(
        await lastValueFrom(service.get(key, { value: 'default' }))
