...
export class FileService implements IContribution {
  ...
  unregisterFolders(...uris: monaco.Uri[]): void {
    this.folders.splice(
      0,
      this.folders.length,
      ...this.folders.filter((f) => !uris.some((uri) => uriID(uri) === uriID(f.uri)))
    )
  ...
  async replaceFolder(oldUri: monaco.Uri, newFolder: IFolder): Promise<void> {
    const index = this.folders.findIndex((f) => uriID(f.uri) === uriID(oldUri))
    if (index === -1) {
  ...
  isRoot(uri: monaco.Uri): boolean {
    return this.folders.some((f) => uriID(f.uri) === uriID(uri))
  }
  ...
  async rename(entry: IFile, newName: string): Promise<void> {
    ...
    const newUri = entry.uri.with({
      path: Paths.normalize(Paths.join([Paths.dirname(entry.uri.path), newName])),
    })

    this.willChangeFile.next([{ type: FileChangeType.Deleted, uri: entry.uri }])
    this.willChangeFile.next([{ type: FileChangeType.Created, uri: newUri }])
  ...
  private async doMove(source: IFile, destination: IFile, copy?: boolean): Promise<void> {
  ...
  private async doCreate(parent: IFile, name: string, asFolder: boolean): Promise<void> {
  ...
  private async withProvider(
    ...
    if (!Paths.isAbsolutePath(uri.path)) {
      throw new Error(`Unable to resolve filesystem provider with relative file path "${uri.toString(true)}"`)
    }

    const provider = this.providers.get(uri.scheme)
    if (!provider) {
      throw new Error(`No file system provider found for ${uri.with({ query: '' }).toString(true)}`)
    }
